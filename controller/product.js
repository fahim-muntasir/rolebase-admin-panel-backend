...
const addProduct = async (req, res) => {
  ...
  if (req.files.length > 0) {
    try {
      ...
      res
        .status(500)
        .json({ errors: { msg: "Something is wrong on user create!" } });
    }
  } else {
    res
      .status(500)
      .json({ errors: { avatar: "Please upload a product image!" } });
  }
};
  ...
  if (req.body?.status) {
    if (req.data.role === "admin") {
      try {
        ...
        res.status(500).json({
          errors: { msg: "Somthing is wrong Product is not updated!" },
        });
      }
    } else {
      res
...
const updateProductImg = async (req, res) => {
  ...
  if (req.files.length > 0) {
    ...
    try {
      ...
      res
        .status(200)
        .json({ success: true, updated_avatar: req.files[0].filename });
    } catch {
      res.status(500).json({
        errors: { msg: "Somthing is wrong Product Img is not updated!" },
      });
    }
  }
};
